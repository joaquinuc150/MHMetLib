...
int main(int argc, char **argv) {
    ...
    if (isPopulationBased) {
        ...
        int hammingDistanceVariantInt = std::stoi(config["hammingDistanceVariant"]);
        HammingDistanceVariantEnum hdVariant = static_cast<HammingDistanceVariantEnum>(hammingDistanceVariantInt);

        route = path + "/";

        outputMetrics(path, n, entropySize, std::vector<Domain>(), hdVariant, maxProblem);
